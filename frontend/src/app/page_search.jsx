// ============================================================
//  page_search.jsx — BookCard + SearchPage (route "/")
// ============================================================
import { useState as useStateS, useEffect as useEffectS, useCallback as useCbS, useRef as useRefS } from 'react'
import { METHODS, EMOTIONS, EMOTION_META, EXAMPLE_QUERIES, apiSearch, apiStats } from './data'
import { CoverImage, StarRating, EmotionBadge, Icon, Button, MethodChip, Spinner, EmptyState, ErrorState, METHOD_EXPLAIN, useAsync } from './ui'

function BookCard({ book, method, onOpen, index = 0 }) {
  const [hover, setHover] = useStateS(false);
  return (
    <button onClick={() => onOpen({ ...book, _method: method })}
      onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{ textAlign: "left", border: "1px solid var(--line)", background: "var(--surface)", cursor: "pointer",
        borderRadius: "var(--r-lg)", padding: 14, display: "flex", flexDirection: "column", gap: 12, fontFamily: "inherit",
        boxShadow: hover ? "var(--shadow-md)" : "var(--shadow-sm)", transform: hover ? "translateY(-3px)" : "none",
        transition: "transform .18s cubic-bezier(.2,.7,.2,1), box-shadow .18s", animation: `fadeUp .4s ${index * 0.03}s both` }}>
      <div style={{ position: "relative" }}>
        <CoverImage book={book} size="card" />
        {book.similarity_score != null && (
          <span className="mono" style={{ position: "absolute", top: 8, right: 8, fontSize: 11, fontWeight: 600,
            padding: "3px 7px", borderRadius: 6, background: "oklch(1 0 0 / 0.92)", color: "var(--accent-ink)",
            border: "1px solid var(--accent-line)", boxShadow: "var(--shadow-sm)" }}>{book.similarity_score.toFixed(2)}</span>
        )}
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 6, flex: 1 }}>
        <div className="serif" style={{ fontSize: 16.5, fontWeight: 600, lineHeight: 1.2, letterSpacing: "-0.01em",
      ...
      {METHODS.map(m => {
        ...
        return (
          <button key={m.id} onClick={() => onChange(m.id)} title={METHOD_EXPLAIN[m.id]}
            style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 13px", cursor: "pointer",
              borderRadius: "var(--r-md)", fontFamily: "inherit", fontSize: 13, fontWeight: 600,
              border: active ? "1px solid var(--accent-line)" : "1px solid var(--line)",
              background: active ? "var(--accent-wash)" : "var(--surface)",
              color: active ? "var(--accent-ink)" : "var(--ink-soft)", transition: "all .14s" }}>
...
function EmotionFilter({ value, onChange }) {
  const toggle = em => onChange(value.includes(em) ? value.filter(x => x !== em) : [...value, em]);
  return (
      ...
      {EMOTIONS.map(em => {
        ...
        return (
          <button key={em} onClick={() => toggle(em)}
            style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "5px 11px", cursor: "pointer",
              borderRadius: 99, fontFamily: "inherit", fontSize: 12.5, fontWeight: 600, transition: "all .14s",
              border: active ? `1px solid ${m.color}` : "1px solid var(--line)",
              background: active ? "var(--paper-2)" : "var(--surface)", color: active ? "var(--ink)" : "var(--ink-mute)" }}>
...
function SearchPage({ onOpenBook }) {
  const [query, setQuery] = useStateS("");
  const [committed, setCommitted] = useStateS(null); // {query, method, emotions, topK}
  const [method, setMethod] = useStateS("semantic");
  const [emotions, setEmotions] = useStateS([]);
  const [topK, setTopK] = useStateS(10);
  const inputRef = useRefS(null);

  const stats = useAsync(() => apiStats(), []);

  const search = useAsync(
    () => apiSearch({ query: committed.query, top_k: committed.topK, method: committed.method, filter_emotions: committed.emotions.length ? committed.emotions : null }),
    [committed && committed.query, committed && committed.method, committed && committed.topK, committed && (committed.emotions || []).join(",")],
    { enabled: !!committed }
  );

  const submit = useCbS((q) => {
  ...
  // re-run when controls change after a first search (live refinement)
  useEffectS(() => { if (committed) setCommitted(c => ({ ...c, method, emotions, topK })); }, [method, emotions, topK]);
      ...
      <div style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: "var(--r-lg)",
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "stretch", flexWrap: "wrap" }}>
          <div style={{ position: "relative", flex: 1, minWidth: 260 }}>
            ...
            <input ref={inputRef} value={query} onChange={e => setQuery(e.target.value)} onKeyDown={onKey}
              ...
              style={{ width: "100%", height: 52, padding: "0 16px 0 46px", fontSize: 15.5, fontFamily: "var(--font-serif)",
                borderRadius: "var(--r-md)", border: "1px solid var(--line)", background: "var(--paper)", color: "var(--ink)", outline: "none" }}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
          ...
          {EXAMPLE_QUERIES.slice(0, 4).map(q => (
            <button key={q} onClick={() => submit(q)} style={{ fontFamily: "var(--font-serif)", fontStyle: "italic",
              fontSize: 13, color: "var(--accent-ink)", background: "transparent", border: "none", cursor: "pointer",
              padding: 0, borderBottom: "1px solid var(--accent-line)" }}>"{q}"</button>
          ))}
        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 24, alignItems: "start" }} className="search-lower">
            ...
            <Field label="Results">
              <div style={{ display: "inline-flex", background: "var(--paper-2)", borderRadius: "var(--r-md)", padding: 3, border: "1px solid var(--line)" }}>
                {[5, 10, 20].map(k => (
                  <button key={k} onClick={() => setTopK(k)} className="mono" style={{ border: "none", cursor: "pointer", padding: "7px 15px",
                    borderRadius: 6, fontSize: 13, fontWeight: 600, background: topK === k ? "var(--surface)" : "transparent",
                    color: topK === k ? "var(--ink)" : "var(--ink-mute)", boxShadow: topK === k ? "var(--shadow-sm)" : "none" }}>{k}</button>
                ))}
              </div>
            </Field>
          </div>
...
function ResultsHeader({ data, committed, loading, fetching }) {
  return (
...
function IdleState({ stats, onPick }) {
  return (
    <div style={{ animation: "fadeIn .4s" }}>
      ...
      {/* stats strip */}
      <div style={{ display: "flex", justifyContent: "center", gap: 0, flexWrap: "wrap", border: "1px solid var(--line)",
        borderRadius: "var(--r-lg)", overflow: "hidden", maxWidth: 620, margin: "0 auto", background: "var(--surface)" }}>
        {stats.loading ? [0, 1, 2].map(i => <div key={i} style={{ flex: 1, padding: "18px 24px" }}><div className="skel" style={{ height: 28, width: "70%" }} /></div>)
          : stats.data && [
            ...
            { v: stats.data.avg_rating.toFixed(2), l: "avg rating" },
          ].map((s, i) => (
            <div key={i} style={{ flex: 1, minWidth: 140, padding: "18px 24px", textAlign: "center",
      ...
      <div style={{ maxWidth: 620, margin: "28px auto 0" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {EXAMPLE_QUERIES.map(q => (
            <button key={q} onClick={() => onPick(q)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
              gap: 12, padding: "13px 16px", borderRadius: "var(--r-md)", border: "1px solid var(--line)", background: "var(--surface)",
              cursor: "pointer", fontFamily: "var(--font-serif)", fontSize: 15, fontStyle: "italic", color: "var(--ink)", textAlign: "left",
...
export { SearchPage, BookCard, SkeletonGrid };
