// ============================================================
//  page_compare.jsx — CompareMethodsPage (route "/compare")
//  Runs all 5 methods in parallel for one shared query.
// ============================================================
import { useState as useStateC2, useCallback as useCbC2, useMemo as useMemoC2 } from 'react'
import { METHODS, METHOD_MAP, BOOK_BY_ISBN, EXAMPLE_QUERIES, apiSearch } from './data'
import { CoverImage, Icon, Button, EmptyState, useAsync } from './ui'

function CompareRow({ book, rank, method, onOpen }) {
  const [hover, setHover] = useStateC2(false);
  return (
    <button onClick={() => onOpen({ ...book, _method: method })}
      onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{ display: "grid", gridTemplateColumns: "18px 34px 1fr", gap: 9, alignItems: "center", width: "100%",
        textAlign: "left", border: "none", borderRadius: "var(--r-sm)", padding: "7px 8px", cursor: "pointer",
        background: hover ? "var(--paper-2)" : "transparent", fontFamily: "inherit", transition: "background .12s" }}>
...
function CompareColumn({ method, state, onOpen, highlightISBNs }) {
        ...
        {state.loading ? (
          Array.from({ length: 5 }).map((_, i) => (
            <div key={i} style={{ display: "flex", gap: 9, padding: "7px 8px", alignItems: "center" }}>
              ...
              <div style={{ flex: 1 }}><div className="skel" style={{ height: 11, width: "90%" }} /><div className="skel" style={{ height: 9, width: "55%", marginTop: 6 }} /></div>
            </div>
          ))
        ) : state.error ? (
          ...
          state.data.results.slice(0, 5).map((b, i) => (
            <div key={b.isbn13} style={{ borderRadius: "var(--r-sm)", outline: highlightISBNs.has(b.isbn13) ? `1.5px solid ${m.color}` : "none",
...
function useParallelSearch(query, topK) {
  return useAsync(async () => {
    const entries = await Promise.all(METHODS.map(async m => {
      try { return [m.id, { data: await apiSearch({ query, top_k: topK, method: m.id, filter_emotions: null }), loading: false, error: null }]; }
      catch (e) { return [m.id, { data: null, loading: false, error: e }]; }
    }));
    return Object.fromEntries(entries);
  }, [query, topK], { enabled: !!query });
}

function computeInsights(states) {
  // symmetric-difference storytelling across method result sets (top-5)
  if (!states) return [];
  const top = {};
  METHODS.forEach(m => { top[m.id] = new Set((states[m.id]?.data?.results || []).slice(0, 5).map(b => b.isbn13)); });
  const titleOf = isbn => (BOOK_BY_ISBN[isbn] || {}).title;
  const out = [];

  // 1. semantic vs tfidf — the headline contrast
  const semList = (states.semantic?.data?.results || []).slice(0, 5);
  const semOnly = semList.filter(b => !top.tfidf.has(b.isbn13));
  if (semOnly.length) out.push({
    a: "semantic", z: "tfidf",
  ...
  // 2. reranking vs hybrid ordering
  const rerank = (states.reranking?.data?.results || []).slice(0, 5);
  const hybrid = (states.hybrid?.data?.results || []).slice(0, 5);
  const rerankOnly = rerank.filter(b => !top.hybrid.has(b.isbn13));
  if (rerankOnly.length) out.push({
  ...
  else if (rerank.length && hybrid.length && rerank[0].isbn13 !== hybrid[0].isbn13) out.push({
    a: "reranking", z: "hybrid",
  ...
  out.push({
    ...
    text: uniqueTop1.size === 1
      ? <>All five methods agree on the same top result — <em>{titleOf(allTop1[0])}</em> — a strong, unambiguous match.</>
...
function CompareMethodsPage({ onOpenBook }) {
  const [query, setQuery] = useStateC2("");
  const [committed, setCommitted] = useStateC2("");
  const states = useParallelSearch(committed, 5);

  const submit = useCbC2(q => { const qq = (q != null ? q : query).trim(); if (!qq) return; if (q != null) setQuery(qq); setCommitted(qq); }, [query]);

  const insights = useMemoC2(() => (states.data ? computeInsights(states.data) : []), [states.data]);
  // books unique to a single method get a subtle outline in their column
  const highlight = useMemoC2(() => {
    const map = {}; if (!states.data) return map;
    const counts = {};
    METHODS.forEach(m => (states.data[m.id]?.data?.results || []).slice(0, 5).forEach(b => { counts[b.isbn13] = (counts[b.isbn13] || 0) + 1; }));
    METHODS.forEach(m => { map[m.id] = new Set((states.data[m.id]?.data?.results || []).slice(0, 5).filter(b => counts[b.isbn13] === 1).map(b => b.isbn13)); });
    return map;
  }, [states.data]);
    ...
    <div style={{ maxWidth: "var(--maxw)", margin: "0 auto", padding: "28px 24px 80px", width: "100%" }}>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "stretch", flexWrap: "wrap", maxWidth: 720 }}>
        <div style={{ position: "relative", flex: 1, minWidth: 240 }}>
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()}
            ...
            style={{ width: "100%", height: 48, padding: "0 16px 0 44px", fontSize: 15, fontFamily: "var(--font-serif)",
              borderRadius: "var(--r-md)", border: "1px solid var(--line)", background: "var(--surface)", color: "var(--ink)", outline: "none" }}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
        ...
        {EXAMPLE_QUERIES.slice(0, 3).map(q => (
          <button key={q} onClick={() => submit(q)} style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: 13,
            color: "var(--accent-ink)", background: "transparent", border: "none", cursor: "pointer", padding: 0, borderBottom: "1px solid var(--accent-line)" }}>"{q}"</button>
        ))}
      </div>
      ...
      {!committed ? (
        ...
        <>
          <div style={{ marginTop: 26 }} className="compare-grid">
            {METHODS.map(m => (
              <CompareColumn key={m.id} method={m.id} state={states.loading ? { loading: true } : (states.data ? states.data[m.id] : { loading: true })}
                onOpen={onOpenBook} highlightISBNs={highlight[m.id] || new Set()} />
            ))}
          </div>
